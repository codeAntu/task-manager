...
function dayCalendarFilter(routine: Routine, date: Date) {
  ...
  const routineDate = new Date(routine.time[0] + 'T00:00');
  const y1 = date.getFullYear();
  const y2 = routineDate.getFullYear();

  const m1 = date.getMonth();
  const m2 = routineDate.getMonth();

  const d1 = date.getDate();
  const d2 = routineDate.getDate();
...
function dayFilterMonthly(routine: Routine, date: Date) {
  ...
  if (currentDate in routine.time) {
     ...
     const endTimeRoutine = timeSplitter(routine.time[currentDate][1]);
...
function dayFilterYearly(routine: Routine, date: Date) {
  ...
  if (routine.time.month === month && routine.time.date === dt) {
     ...
     const endTimeRoutine = timeSplitter(routine.time.time[1]);
     if (startTimeRoutine) routine.startTime = getNewDateSetHM(date, startTimeRoutine);
     if (endTimeRoutine) routine.endTime = getNewDateSetHM(date, endTimeRoutine);
     return true;
  }
...
function dayFilterWeekly(routine: Routine, date: Date) {
  ...
  if (currentDay in routine.time) {
     ...
     const endTimeRoutine = timeSplitter(routine.time[currentDay][1]);
     if (startTimeRoutine) routine.startTime = getNewDateSetHM(date, startTimeRoutine);
     if (endTimeRoutine) routine.endTime = getNewDateSetHM(date, endTimeRoutine);
     return true;
  }
...
function dayFilterDaily(routine: Routine, date: Date) {
  ...
  const endTimeRoutine = timeSplitter(routine.time[1]);
  if (startTimeRoutine) routine.startTime = getNewDateSetHM(date, startTimeRoutine);
  if (endTimeRoutine) routine.endTime = getNewDateSetHM(date, endTimeRoutine);
  return true;
}
